...
export const test_assertStringify_TagLength = _test_assertStringify("TagLength", TagLength.generate, (input) => ((input: any): string => { const assert = (input: any): Array<TagLength.Type> => {
    ...
    return `[${input.map((elem: any) => `{"fixed":${$string((elem as any).fixed)},"minimum":${$string((elem as any).minimum)},"maximum":${$string((elem as any).maximum)},"minimum_and_maximum":${$string((elem as any).minimum_and_maximum)}}`).join(",")}]`;
