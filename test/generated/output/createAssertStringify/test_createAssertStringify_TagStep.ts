...
export const test_createAssertStringify_TagStep = _test_assertStringify("TagStep", TagStep.generate, (input: any): string => { const assert = (input: any): TagStep => {
    ...
    return `[${input.map((elem: any) => `{"exclusiveMinimum":${$number((elem as any).exclusiveMinimum)},"minimum":${$number((elem as any).minimum)},"range":${$number((elem as any).range)},"multipleOf":${$number((elem as any).multipleOf)}}`).join(",")}]`;
