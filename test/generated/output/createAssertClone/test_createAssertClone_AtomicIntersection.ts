...
export const test_createAssertClone_AtomicIntersection = _test_assertClone("AtomicIntersection", AtomicIntersection.generate, (input: any): typia.Primitive<AtomicIntersection> => { const assert = (input: any): AtomicIntersection => {
    ...
    if (false === __is(input))
        ((input: any, _path: string, _exceptionable: boolean = true): input is AtomicIntersection => {
    ...
    return input;
}; const clone = (input: AtomicIntersection): typia.Primitive<AtomicIntersection> => {
    return Array.isArray(input) && (input.length === 3 && "boolean" === typeof input[0] && "number" === typeof input[1] && "string" === typeof input[2]) ? [
        input[0] as any,
        input[1] as any,
        input[2] as any
    ] as any : input as any;
}; assert(input); const output = clone(input); return output; }, AtomicIntersection.SPOILERS);
